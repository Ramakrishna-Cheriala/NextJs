...
const UserDetails = ({ type, id, subAccounts, userData }: Props) => {
  ...
  useEffect(() => {
    if (data.user) {
      const fetchDetails = async () => {
        const responce = await getAuthUserDetails();
        if (responce) setAuthUserData(responce);
      };
  ...
  const onChangePermission = async (
    ...
    const responce = await changeUserPermissions(
    ...
    if (type === "agency") {
      await saveActivityLogsNotification({
        ...
        description: `Gave ${userData?.email} access to | ${
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Add or update the information</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
